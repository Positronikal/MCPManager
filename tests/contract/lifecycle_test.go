...
func TestPostServerStart_ContractValidation(t *testing.T) {
	...
	t.Run("should return 400 if server already running", func(t *testing.T) {
		...
		if state != "running" {
			t.Skipf("Server did not reach running state (reached %q), skipping already-running test", state)
		}
...
func TestPostServerStop_ContractValidation(t *testing.T) {
	...
	t.Run("should return 202 for graceful stop", func(t *testing.T) {
		...
		state := waitForStableState(t, router, validUUID, 5*time.Second)
		if state != "running" {
			t.Skipf("Server did not reach running state (reached %q), skipping stop test", state)
		}
	...
	t.Run("should return 400 if server not running", func(t *testing.T) {
		...
		// Ensure server is stopped before testing
		ensureServerStopped(t, router, validUUID)
